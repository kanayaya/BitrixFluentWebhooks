...
// Методы событий: https://dev.1c-bitrix.ru/rest_help/general/events_method/index.php
     ...
     * <li><b>{@code event_type}</b> - Значения: online|offline. По умолчанию event_type=online, и поведение метода не меняется. Если казывается event_type=offline, то метод работает с офлайн событиями.</li>
     * <li><b>{@code event}</b> - Имя события. Обязательный.</li>
     * </ul>
     */
...
// Настройка приложений: https://dev.1c-bitrix.ru/rest_help/general/application/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code option}</b> - Строка, один из ключей из свойства app.option.set.</li>
     * </ul>
     */
...
// Лиды: https://dev.1c-bitrix.ru/rest_help/crm/leads/index.php
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор лида.</li>
     * <li><b>{@code rows}</b> - Товарные позиции - массив вида array(array("поле"=>"значение"[, ...])[, ...]), где "поле" может принимать значения из возвращаемых методом crm.productrow.fields. Товарные позиции лида, существующие до момента вызова метода, будут заменены новыми. После сохранения будет произведён пересчёт суммы лида.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор лида.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.lead.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.lead.fields и посмотрите формат пришедших значений этих полей.</li>
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор пользовательского поля.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.userfield.fields.</li>
     * <li><b>{@code LIST}</b> - Содержит набор значений списка для пользовательских полей типа Список. Указывается при создании/обновлении поля. Каждое значение представляет собой массив с полями: VALUE - значение элемента списка. Поле является обязательным в случае, когда создается новый элемент. SORT - сортировка. DEF - если равно Y, то элемент списка является значением по-умолчанию. Для множественного поля допустимо несколько DEF=Y. Для не множественного, дефолтным будет считаться первое. XML_ID - внешний код значения. Параметр учитывается только при обновлении уже существующих значений элемента списка. ID - идентификатор значения. Если он указан, то считается что это обновление существующего значения элемента списка, а не создание нового. Имеет смысл только при вызове методов *.userfield.update. DEL - если равно Y, то существующий элемент списка будет удален. Применяется, если заполнен параметр ID.</li>
     * </ul>
     */
...
// Компании: https://dev.1c-bitrix.ru/rest_help/crm/company/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), содержащий значения полей компании. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.company.fields и посмотрите формат пришедших значений этих полей.</li>
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор компании.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.company.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.company.fields и посмотрите формат пришедших значений этих полей.</li>
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор пользовательского поля.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.userfield.fields.</li>
     * <li><b>{@code LIST}</b> - Содержит набор значений списка для пользовательских полей типа Список. Указывается при создании/обновлении поля. Каждое значение представляет собой массив с полями: VALUE - значение элемента списка. Поле является обязательным в случае, когда создается новый элемент. SORT - сортировка. DEF - если равно Y, то элемент списка является значением по-умолчанию. Для множественного поля допустимо несколько DEF=Y. Для не множественного, дефолтным будет считаться первое. XML_ID - внешний код значения. Параметр учитывается только при обновлении уже существующих значений элемента списка. ID - идентификатор значения. Если он указан, то считается что это обновление существующего значения элемента списка, а не создание нового. Имеет смысл только при вызове методов *.userfield.update. DEL - если равно Y, то существующий элемент списка будет удален. Применяется, если заполнен параметр ID.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор компании.</li>
     * <li><b>{@code fields}</b> - Объект со следующими полями: CONTACT_ID - идентификатор контакта (обязательное поле) SORT - индекс сортировки IS_PRIMARY - флаг первичного контакта Примечание: чтобы узнать требуемый формат полей, выполните метод crm.company.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Контакты: https://dev.1c-bitrix.ru/rest_help/crm/contacts/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей – массив вида array("поле"=>"значение"[, ...]), содержащий значения полей контакта. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.contact.fields и посмотрите формат пришедших значений этих полей.</li>
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор контакта.</li>
     * <li><b>{@code fields}</b> - Объект со следующими полями: COMPANY_ID - идентификатор компании (обязательное поле) SORT - индекс сортировки IS_PRIMARY - флаг первичной компании Примечание: чтобы узнать требуемый формат полей, выполните метод crm.contact.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор контакта.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.contact.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.contact.fields и посмотрите формат пришедших значений этих полей.</li>
...
// Сделки: https://dev.1c-bitrix.ru/rest_help/crm/cdeals/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), содержащий значения полей сделки. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.deal.fields и посмотрите формат пришедших значений этих полей.</li>
     ...
     * <li><b>{@code fields}</b> - Объект со следующими полями: CONTACT_ID - идентификатор контакта (обязательное поле) SORT - индекс сортировки IS_PRIMARY - флаг первичного контакта Примечание: чтобы узнать требуемый формат полей, выполните метод crm.deal.contact.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
     ...
     * <li><b>{@code rows}</b> - Товарные позиции - массив вида array(array("поле"=>"значение"[, ...])[, ...]), где "поле" может принимать значения из возвращаемых методом crm.productrow.fields. Товарные позиции сделки, существующие до момента вызова метода, будут заменены новыми. После сохранения будет произведён пересчёт суммы сделки.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор настройки шаблона регулярной сделки.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.deal.recurring.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.deal.recurring.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор сделки.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.deal.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.deal.fields и посмотрите формат пришедших значений этих полей.</li>
...
// Статусы счетов: https://dev.1c-bitrix.ru/rest_help/crm/invoice_status/index.php
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор статуса счёта.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.invoice.status.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.invoice.status.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Предложения: https://dev.1c-bitrix.ru/rest_help/crm/komm_quote/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), содержащий значения полей предложения, где "поле" может принимать значения из возвращаемых методом crm.quote.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.quote.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор предложения.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.quote.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.quote.fields и посмотрите формат пришедших значений этих полей.</li>
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор предложения.</li>
     * <li><b>{@code rows}</b> - Товарные позиции - массив вида array(array("поле"=>"значение"[, ...])[, ...]), где "поле" может принимать значения из возвращаемых методом crm.productrow.fields. Товарные позиции предложения, существующие до момента вызова метода, будут заменены новыми. После сохранения будет произведён пересчёт суммы предложения.</li>
     * </ul>
     */
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), содержащий описание пользовательского поля. Полное описание полей можно получить вызовом метода crm.userfield.fields.</li>
     * </ul>
     */
...
// Элементы смарт-процессов: https://dev.1c-bitrix.ru/rest_help/crm/dynamic/methodscrmitem/index.php
    ...
    /**
     * Создаёт создает новый элемент смарт-процесса.<br>
     * Параметры:<br>
     * <ul>
     * <li><b>{@code entityTypeId}</b> - Идентификатор смарт-процесса.</li>
...
// Комментарии: https://dev.1c-bitrix.ru/rest_help/crm/timeline/comment/index.php
     ...
     * <li><b>{@code fields}</b> - Набор полей - массив вида array ("поле"=>"значение"[, ...]), содержащий значения полей. Доступные значения - COMMENT и FILES. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.timeline.comment.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Дела: https://dev.1c-bitrix.ru/rest_help/crm/rest_activity/index.php
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор активности.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.activity.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.activity.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Методы: https://dev.1c-bitrix.ru/rest_help/crm/requisite/methods/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]). Где "поле" может принимать значения из возвращаемых методом crm.address.fields.</li>
     * </ul>
     */
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), содержащий значения полей адреса. Где "поле" может принимать значения из возвращаемых методом crm.requisite.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.requisite.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), содержащий значения полей адреса. Где "поле" может принимать значения из возвращаемых методом crm.requisite.bankdetail.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.requisite.bankdetail.fields и посмотрите формат пришедших значений этих полей. Внимание! В поле ENTITY_ID указывается не ID контакта/компании, а ID реквизита.</li>
     * </ul>
     */
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code entityTypeId}</b> - Идентификатор типа сущности (см. метод crm.enum.ownertype). Для связей реквизитов может быть только сделка, предложение или счёт.</li>
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), содержащий значения полей шаблона. Где "поле" может принимать значения из возвращаемых методом crm.requisite.preset.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.requisite.preset.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор реквизита.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.requisite.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.requisite.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Шаблоны документов: https://dev.1c-bitrix.ru/rest_help/crm/documentgenerator/tempates/index.php
     ...
     * <ul>
     * <li><b>{@code id}</b> - идентификатор шаблона.</li>
     * <li><b>{@code fields}</b> - массив полей. Аналогично методу crm.documentgenerator.template.add, только здесь все поля необязательные.</li>
     * </ul>
     */
...
// Пользовательские поля: https://dev.1c-bitrix.ru/rest_help/crm/userfields/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code type}</b> - Tип пользовательского поля. Значение из списка возвращаемого методом crm.userfield.types.</li>
     * </ul>
     */
...
// Валюты: https://dev.1c-bitrix.ru/rest_help/crm/currency/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code localizations}</b> - Набор локализаций - массив вида array("язык" => array("поле"=>"значение"[, ...])), содержащий значения полей локализаций, где "язык" - идентификатор языка, "поле" - одно из возвращаемых методом crm.currency.localizations.fields значений.</li>
     * <li><b>{@code id}</b> - Символьный идентификатор валюты.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Символьный идентификатор валюты.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.currency.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.currency.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Единицы измерения: https://dev.1c-bitrix.ru/rest_help/crm/measure/index.php
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор единицы измерения.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.measure.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.measure.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Разделы товаров: https://dev.1c-bitrix.ru/rest_help/crm/product_section/index.php
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор раздела товаров.</li>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("обновляемое поле"=>"значение"[, ...]), где "обновляемое поле" может принимать значения из возвращаемых методом crm.productsection.fields. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.productsection.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Товарные позиции: https://dev.1c-bitrix.ru/rest_help/crm/productrow/index.php
     ...
     * <li><b>{@code productRows}</b> - Массив товарных позиций. Каждый элемент массива - объект с данными, аналогичными fields из crm.item.productrow.add.</li>
     * </ul>
     */
...
// Товары: https://dev.1c-bitrix.ru/rest_help/crm/products/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Набор полей - массив вида array("поле"=>"значение"[, ...]), содержащий значения полей товара. Необходимо обязательно указать CURRENCY_ID для установки цены. Примечание: чтобы узнать требуемый формат полей, выполните метод crm.product.fields и посмотрите формат пришедших значений этих полей.</li>
     * </ul>
     */
...
// Шаблоны Бизнес-процессов: https://dev.1c-bitrix.ru/rest_help/bizproc/bizproc_workflow_template/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code FIELDS}</b> - Массив изменяемых параметров. Обновить можно поля: NAME, DESCRIPTION, AUTO_EXECUTE, TEMPLATE_DATA. При попытке обновить другие поля возвращаемые bizproc.workflow.template.list - ошибок не возникнет, но и обновлено ничего не будет.</li>
...
// Пользовательские страны: https://dev.1c-bitrix.ru/rest_help/documentgenerator/region/index.php
     ...
     * <ul>
     * <li><b>{@code id}</b> - ID региона</li>
     * <li><b>{@code fields}</b> - Массив полей. Аналогично методу documentgenerator.region.add, только здесь все поля необязательные.</li>
     * </ul>
     */
...
// item: https://dev.1c-bitrix.ru/rest_help/tasks/task/item/index.php
     ...
     * <ul>
     * <li><b>{@code TASKID}</b> - Идентификатор задачи.</li>
     * <li><b>{@code ACTIONID}</b> - Идентификатор проверяемого действия (см. константы метода task.item.getallowedactions).</li>
     * </ul>
     */
...
// Бэклог: https://dev.1c-bitrix.ru/rest_help/tasks/task/scrum/backlog/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Поля, соответствующие доступному списку полей tasks.api.scrum.backlog.getFields. Поле groupId обязательное.</li>
     * </ul>
     */
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор бэклога. Обязательный параметр.</li>
     * <li><b>{@code fields}</b> - Поля, соответствующие доступному списку полей tasks.api.scrum.backlog.getFields.</li>
     * </ul>
     */
...
// Спринты: https://dev.1c-bitrix.ru/rest_help/tasks/task/scrum/sprint/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Поля, соответствующие доступному списку полей tasks.api.scrum.sprint.getFields. Поле groupId обязательное. Доступные значения для полей дат dateStart и dateEnd: ('ISO 8601', timestamp). Доступные значения для поля status: ('active', 'planned', 'completed').</li>
     * </ul>
     */
...
// Эпики: https://dev.1c-bitrix.ru/rest_help/tasks/task/scrum/epic/index.php
     ...
     * <li><b>{@code fields}</b> - Поля, соответствующие доступному списку полей tasks.api.scrum.epic.getFields. В поле files можно передать массив значений с идентификаторами файлов, указав префикс n. Если в files передать пустой массив, файлы удалятся.</li>
     * </ul>
     */
...
// Задачи скрама: https://dev.1c-bitrix.ru/rest_help/tasks/task/scrum/task/index.php
     ...
     * <ul>
     * <li><b>{@code id}</b> - Идентификатор задачи.</li>
     * <li><b>{@code fields}</b> - Поля, соответствующие доступному списку полей tasks.api.scrum.task.getFields.</li>
     * </ul>
     */
...
// Канбан: https://dev.1c-bitrix.ru/rest_help/tasks/task/scrum/kanban/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Поля, соответствующие доступному списку полей tasks.api.scrum.kanban.getFields. Поля name и sprintId обязательные. Поле sort кратно 100. Доступные значения для поля type: (NEW, WORK, FINISH).</li>
     * </ul>
     */
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code fields}</b> - Поля, соответствующие доступному списку полей tasks.api.scrum.kanban.getFields. Доступные для обновления поля: (name, sprintId, sort, type, color). Необновляемые поля можно не передавать. Поле sort кратно 100. Доступные значения поля для type: (NEW, WORK, FINISH).</li>
     * <li><b>{@code stageId}</b> - Идентификатор стадии.</li>
     * </ul>
     */
...
// Корзина: https://dev.1c-bitrix.ru/rest_help/sale/basketitem/index.php
     ...
     * <li><b>{@code fields: object}</b> - Поля, соответствующие доступному списку полей sale.basketItem.getFieldsCatalogProduct.</li>
     * </ul>
     */
...
// Платёжные системы: https://dev.1c-bitrix.ru/rest_help/paysystem/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code params}</b> - Массив параметров Список доступных полей идентичен полям метода sale.paysystem.add для выборки (select, order, filter) (необязательный).</li>
     * </ul>
     */
...
// Методы для работы с сущностью Страница: https://dev.1c-bitrix.ru/rest_help/landing/page/methods/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code code}</b> - Идентификатор шаблона для создания. Список шаблонов вы можете получить методом landing.demos.getPageList.</li>
...
// Методы для работы с сущностью Блоки: https://dev.1c-bitrix.ru/rest_help/landing/block/blocks/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code data}</b> - Массив для изменения. Для пояснения смотрите пример. Опционально можно передавать пресеты карточек. Обратите внимание, селекторы формируются по похожему методу формирования в landing.block.updatenodes.</li>
     * <li><b>{@code lid}</b> - Идентификатор страницы.</li>
     * <li><b>{@code block}</b> - Идентификатор блока.</li>
     * </ul>
     */
...
// Партнерские шаблоны: https://dev.1c-bitrix.ru/rest_help/landing/demos/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code data}</b> - Результат метода landing.site.fullExport как есть.</li>
...
// telephony: https://dev.1c-bitrix.ru/rest_help/scope_telephony/telephony/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code USER_ID: int}</b> - Идентификатор пользователя. Обязательный. Ответственный в лиде будет изменен на переданный USER_ID. Ответственный будет меняться только для лида, созданного автоматически при вызове метода telephony.externalcall.register. Для существующего лида ответственный не меняется.</li>
     * <li><b>{@code STATUS_CODE: string}</b> - Код вызова (см. таблицу кодов вызова).</li>
     * <li><b>{@code COST_CURRENCY: string}</b> - Валюта, в которой указана стоимость звонка. Список валют можно получить методом crm.currency.list.</li>
...
// voximplant: https://dev.1c-bitrix.ru/rest_help/scope_telephony/voximplant/index.php
     ...
     * Параметры:<br>
     * <ul>
     * <li><b>{@code LINE_ID}</b> - Идентификатор линии, полученный от метода voximplant.line.get или voximplant.line.outgoing.get.</li>
     * </ul>
     */
     ...
     * <li><b>{@code VOICE}</b> - Голос, которым произнести этот текст (необязательный). Список голосов можно получить методом voximplant.tts.voices.get.</li>
...
// Право "Хранилище данных": https://dev.1c-bitrix.ru/rest_help/entity/index.php
     ...
     * <li><b>{@code PROPERTY_VALUES}</b> - Ассоциативный список значений свойств элемента. Свойства хранилища создаются при помощи entity.item.property.add.</li>
     * <li><b>{@code ENTITY}</b> - Обязательный. Строковой идентификатор хранилища.</li>
     ...
     * <li><b>{@code CODE}</b> - Символьный код элемента.</li>
     * <li><b>{@code PROPERTY_VALUES}</b> - Обязательный. ассоциативный список значений свойств элемента. Свойства хранилища создаются при помощи entity.item.property.add.</li>
     * <li><b>{@code ENTITY}</b> - Обязательный. Строковой идентификатор хранилища.</li>
     * <li><b>{@code DETAIL_PICTURE}</b> - Детальная картинка элемента.</li>
     * <li><b>{@code SORT}</b> - Сортировочный вес элемента.</li>
